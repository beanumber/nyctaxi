...
load("~/Desktop/SMITH/nyctaxi/data/green_2016_01_sample.rda")
head(green_2016_01_sample)
green_2016_01_sample <- green_2016_01_sample %>%
  filter(is.na(green_2016_01_sample$pickup_longitude) == FALSE) %>%
  filter(is.na(green_2016_01_sample$pickup_latitude) == FALSE) %>%
  filter(is.na(green_2016_01_sample$pickup_longitude) == FALSE) %>%
  filter(is.na(green_2016_01_sample$pickup_longitude) == FALSE)
```

### Visualization
We can use `leaflet` to visualize the pickup and dropoff locations of the 100 trips in the green taxi trip dataset. There are also shapefiles of NYC Taxi zones attached to `nyctaxi` R package. Let's plot them:

```{r}
m = leaflet(green_2016_01_sample) %>% addTiles()
m %>% addCircleMarkers(~green_2016_01_sample$pickup_longitude, ~green_2016_01_sample$pickup_latitude, radius = 2)
